...
@TRANSFORMS.register_module()
class ThreePackDetInputs(BaseTransform):
    """Pack the inputs data for the detection / semantic segmentation /
    panoptic segmentation.
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        packed_results = dict()
        if 'img' in results and 'img2' and 'img3' in results:
            img = results['img']
            img2 = results['img2']
            img3 = results['img3']
            if len(img.shape) < 3:
            ...
            if not img3.flags.c_contiguous:
                img3 = np.ascontiguousarray(img3.transpose(2, 0, 1))
                img3 = to_tensor(img3)
            else:
                img3 = to_tensor(img3).permute(2, 0, 1).contiguous()

            # print(img.shape, img2.shape)
            # cv2.imwrite("debug1.jpg", img.permute((1,2,0)).cpu().numpy())
            # cv2.imwrite("debug2.jpg", img2.permute((1,2,0)).cpu().numpy())
            # raise RuntimeError()
            packed_results['inputs'], packed_results['inputs2'], packed_results['inputs3'] = img, img2, img3
